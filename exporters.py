...
class PDFExporter:
    ...
    def generate(intelligence_object, output_dir=None):
        ...
        doc_type_label = "INTELLIGENCE DOSSIER"
        if meta.get('is_memo'): doc_type_label = "EXECUTIVE MEMO"
        if meta.get('is_paper'): doc_type_label = "RESEARCH WHITE PAPER"

        safe_title = _safe_filename_part(meta.get('title') or 'Intelligence')
        prefix = "KORUM-OS_DOSSIER"
        if meta.get('is_memo'): prefix = "KORUM-OS_MEMO"
        if meta.get('is_paper'): prefix = "KORUM-OS_WHITE_PAPER"

        filename = f"{prefix}_{safe_title}_{_timestamp()}.pdf"
        ...
        if os.path.exists(logo_path):
            ...
            header_tab_data = [[img, Paragraph(doc_type_label, styles['ExecSig'])]]
        else:
            header_tab_data = [[Paragraph("KORUM-OS", styles['ExecTitle']), Paragraph(doc_type_label, styles['ExecSig'])]]
        ...
        doc_title = _as_text(meta.get('title')).upper() or 'NODE_COMMAND'
        story.append(Paragraph(f"{escape(doc_title)}", styles['ExecTitle']))
        ...
        ctx_data = [
            ...
            [Paragraph("MISSION DIRECTIVE:", styles['ExecLabel']), Paragraph(escape(workflow_label), styles['ExecValue'])],
        ...
        try:
            ...
            # Re-raise with better context for the backend logger
...
class WordExporter:
    ...
    def generate(intelligence_object, output_dir=None):
        ...
        theme_id = _as_text(meta.get("theme")).upper() or "NEON_DESERT"
        if theme_id not in THEMES: theme_id = "NEON_DESERT"
        ...
        labels = [("PREPARED FOR:", client_name.upper()),
                  ("MISSION DIRECTIVE:", _as_text(meta.get('workflow') or 'STRATEGIC_INTEL').upper()),
...
class PPTXExporter:
    ...
    def generate(intelligence_object, output_dir=None):
        ...
        title.text = _as_text(meta.get("title") or "INTELLIGENCE BRIEF").upper()
        subtitle.text = f"KORUM-OS COMMAND NODE // {_as_text(meta.get('workflow') or 'RESEARCH').upper()} // {datetime.now().strftime('%Y-%m-%d')}"
...
class ExecutiveMemoExporter:
    """PDF Executive Memo — Hard-hitting decision summary."""
    @staticmethod
    def generate(intelligence_object, output_dir=None):
        # Implementation of PDF Memo that looks like the Word memo but in PDF
        # For now, it leverages PDFExporter but with a localized focus or theme
        if 'meta' not in intelligence_object: intelligence_object['meta'] = {}
        intelligence_object['meta']['is_memo'] = True
        return PDFExporter.generate(intelligence_object, output_dir=output_dir)

class ResearchPaperExporter:
    """Academic-Grade Technical Research Paper (PDF)."""
    @staticmethod
    def generate(intelligence_object, output_dir=None):
        if 'meta' not in intelligence_object: intelligence_object['meta'] = {}
        intelligence_object['meta']['is_paper'] = True
        return PDFExporter.generate(intelligence_object, output_dir=output_dir)
