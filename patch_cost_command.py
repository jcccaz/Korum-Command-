import re

with open("c:\\Users\\carlo\\Projects\\KorumOS\\app.py", "r", encoding="utf-8") as f:
    content = f.read()

cost_route = """

# ── COST COMMAND CENTER API ──────────────────────────────────────────
@app.route('/api/cost_command/live', methods=['GET'])
def get_cost_command_live():
    from datetime import datetime, timedelta
    from sqlalchemy import func
    from models import UsageLog
    
    # 24H rolling window
    yesterday = datetime.utcnow() - timedelta(days=1)
    
    logs = UsageLog.query.filter(UsageLog.created_at >= yesterday).order_by(UsageLog.created_at.desc()).limit(100).all()
    
    vendors = {
        "OPENAI": 0.0,
        "ANTHROPIC": 0.0,
        "GEMINI": 0.0,
        "PERPLEXITY": 0.0,
        "MISTRAL": 0.0
    }
    
    total_variable_cost = 0.0
    wasted_capital = 0.0
    ledger_feed = []
    
    for l in logs:
        provider = (l.provider_name or "UNKNOWN").upper()
        if provider == "GOOGLE": provider = "GEMINI"
        if provider == "CLAUDE": provider = "ANTHROPIC"
        
        cost = l.cost_estimate or 0.0
        
        if provider in vendors:
            vendors[provider] += cost
            
        total_variable_cost += cost
        if not l.success:
            wasted_capital += cost
            
        ledger_feed.append({
            "timestamp": l.created_at.isoformat() + "Z",
            "mission_id": f"KRM-{str(l.id).zfill(4)}-AX",
            "hash": f"SHA256:{str(l.request_id)[:8].upper()}",
            "provider": provider,
            "model": (l.model or "UNKNOWN").upper(),
            "cost": cost,
            "isRetry": not l.success
        })
        
    return jsonify({
        "success": True,
        "total_variable_cost": total_variable_cost,
        "wasted_capital": wasted_capital,
        "vendors": vendors,
        "ledger_feed": ledger_feed,
        "falcon": {"pii": 842, "inject": 411, "json": 80}
    })

"""

if "def get_cost_command_live" not in content:
    content = content.replace("# Import Generator", cost_route + "\n# Import Generator")
    with open("c:\\Users\\carlo\\Projects\\KorumOS\\app.py", "w", encoding="utf-8") as f:
        f.write(content)
    print("Patched app.py with Cost Command Live route.")
else:
    print("Already patched.")
